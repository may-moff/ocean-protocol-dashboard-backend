...
var addUserKeyValue = function (arr, userKeys, separator) {
    ...
    for (var i = 0; i < arr.length; i++) {
        for (var j = 0; j < userKeys.length; j++) {
            ...
            if (arr[i].toLocaleLowerCase().includes(userKeys[j].toLocaleLowerCase())) {
                var newKeyValue = arr[i].replace(userKeys[j], "" + userKeys[j] + separator);
...
var basicKeyValueSplit = function (arr, separator, whitelist) {
    ...
    var userKeyValues = addUserKeyValue(input, userKeys, separator);
